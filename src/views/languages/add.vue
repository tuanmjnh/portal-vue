            ...
            <v-tabs v-model="$store.state.languages.tabs" color="secondary" dark>
              ...
              <v-tab-item>
                <v-layout wrap class="pt-2">
                  <v-flex xs12 sm8 md8>
                    <v-text-field v-model.trim="item.title" :label="$languages.get('languages.title')"
                  ...
                  <v-flex xs12 sm4 md4>
                    <v-text-field v-model.trim="item.code" class="text-color-initial"
                      ...
                      :rules="[v => !!v  || $languages.get('error.required'),$store.state.languages.exist_code||$languages.get('error.exist')]"></v-text-field>
                  </v-flex>
                  ...
                  <v-flex xs6 sm6 md6>
                    <v-text-field v-model.trim="item.attach_file" :disabled="true" class="text-color-initial"
                      :label="$languages.get('global.attach')"></v-text-field>
                  </v-flex>
                  <v-flex xs6 sm6 md4>
                  ...
                  <v-flex xs12 sm6 md4 v-if="item.code">
                    <upload-files @handleUpload="uploadFiles=$event" :buttonUse="false"
                      ...
                      :buttonText="$languages.get('global.upload_drag')"></upload-files>
                  </v-flex>
                </v-layout>
              </v-tab-item>
              <v-tab-item>
