...
export default {
  ...
  actions: {
          ...
          if (res.status === 200) {
            if (res.data.msg === 'danger') {
              commit(
                SET_MESSAGE,
                {
                  text: rootGetters.languages('error.data'),
                  color: res.data.msg
                },
                { root: true }
              )
    ...
    async selectByLang(
      ...
      await vnptbkn
        .get(`${collection}/GetByLanguage/${state.lang_code}`)
        .then(function (res) {
          if (res.status === 200) {
            if (res.data.msg === 'danger') {
        ...
        .then(function (res) {
          if (res.status == 200) {
            ...
            commit(
              SET_MESSAGE,
              {
                text: rootGetters.languages('success.add'),
                color: res.data.msg
              },
              { root: true }
            )
        ...
        .then(function (res) {
          if (res.status == 200) {
            ...
            commit(
              SET_MESSAGE,
              {
                text: rootGetters.languages('success.update'),
                color: res.data.msg
              },
              { root: true }
            )
    ...
    async delete({ commit, state, rootGetters, rootState }, loading = true) {
      ...
      await vnptbkn
        .put(
          `${collection}/delete`,
          state.selected.map(x => ({ id: x.id, flag: x.flag === 0 ? 1 : 0 }))
        )
        .then(function (res) {
          if (res.status == 200) {
            if (res.data.msg === 'danger') {
            ...
            commit(
              SET_MESSAGE,
              {
                text: rootGetters.languages('success.delete'),
                color: res.data.msg
              },
              { root: true }
            )
