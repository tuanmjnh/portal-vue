  ...
  <div>
    <v-card>
      <v-card-title>
        <v-layout wrap>
          <v-flex xs12 sm4 md4 class="pr-3">
            <v-select :items="app_key" v-model="pagination.app_key" :label="$store.getters.languages('category.app_key')"
      ...
      <v-data-table class="elevation-1" v-model="$store.state.news.selected" select-all
        ...
        :pagination.sync="pagination" :loading="$store.state.$loadingGet" :total-items="$store.state.news.totalItems">
