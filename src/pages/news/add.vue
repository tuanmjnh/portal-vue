        ...
        <v-form v-model="valid" ref="form">
          <v-container grid-list-md>
            <v-tabs v-model="tabs" color="secondary" dark>
              ...
              <v-tab>{{$store.getters.languages('global.files')}}</v-tab>
              <v-tab>{{$store.getters.languages('global.images')}}</v-tab>
              <v-tab>{{$store.getters.languages('global.contents')}}</v-tab>
              <v-tab-item>
                <v-layout wrap class="pt-2">
                  <v-flex xs12 sm5 md5>
                    <v-select :items="category" v-model.trim="item.code" item-value="id"
                      item-text="title" :label="$store.getters.languages('global.dependent')"
                  ...
                  <v-flex xs12 sm7 md7>
                    <v-text-field v-model.trim="item.title" :rules="[v=>!!v||$store.getters.languages('error.required')]"
                      :label="$store.getters.languages('news.title')"></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm8 md8>
                    <v-text-field v-model.trim="item.url" :label="$store.getters.languages('global.url')"></v-text-field>
                  </v-flex>
                  <v-flex xs3 sm1 md1>
                  ...
                  <v-flex xs3 sm1 md1>
                    <v-text-field type="number" v-model.trim="item.quantity" :label="$store.getters.languages('global.quantity')"></v-text-field>
                  </v-flex>
                  <v-flex xs6 sm2 md2>
                  ...
                  <v-flex xs12 sm6 md6 class="text-append-icon">
                    <v-text-field v-model.trim="tag" :label="$store.getters.languages('global.tags')"></v-text-field>
                    <v-tooltip bottom>
